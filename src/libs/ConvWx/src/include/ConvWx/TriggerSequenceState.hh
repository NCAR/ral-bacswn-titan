...
public:
  ...
  Update_t update(const time_t gt, const int lt, time_t &oldGt, const std::vector<int> &leadSecs);

  Update_t update(const time_t gt, const int lt, time_t &oldGt, const std::vector<int> &leadSecs, bool isLast);
...
private:
  ...
  /**
   * Lead times expected from init 
   */
  std::vector<int> pLts;

  /**
   * Status of each lead time 
  ...
  int pLeadSecondsIndex( const int lt);

  void pReset( const std::vector<int> ltSecs);
