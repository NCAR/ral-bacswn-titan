...
public:
   ...
   *
   * The generation time rate is unknown when uniqueIntraHourLetSets is false
   *
   * @param[in] name  Short name
   * @param[in] descr Longer description
   * @param[in] isRequired  True if the data type is required
  ...
  ParmFcst(const std::string &name, const std::string &descr,
	   ...
	   const bool remap, const int genDeltaMinutes, 
           const bool uniqueIntraHourLeadSets = false);
  ...
  /** 
   * Set unique intra-hour generation time flag to true, calculate the gen times per hour
   */
  void setUniqueIntraHourLeadSets();
  ...
  void setDetails(const std::string &url, const std::string &field,
		  const std::string &units, const double hour0,
		  const double hour1, const double deltaHours,
		  const bool remap, const bool uniqueIntraHourLeadSets = false);
#endif
  ...
  {
     // note consistency checking make sure this divides exactly
     return (pLtSeconds1-pLtSeconds0)/pLtDeltaSeconds + 1;
  }

  /**
   * @return The number of lead times specified by parameters
   */
  int nlead(const time_t gt) const;
  ...
  {
    return (double)(ithLeadSeconds(i))/convWx::DOUBLE_SECS_PER_HOUR;
  }
  
  /**
   * @return ith lead hour for generation time gt
   * @param[in] i  Index of lead hour
   */
  double ithLeadHours(const time_t gt, const int i) const;
  ...
  {
    return pLtSeconds0 + i*pLtDeltaSeconds;
  }

  /**
   * Set ith lead seconds for generation time gt
   * @param[in] gt  Generation time
   * @param[in] i  Index of lead seconds
   * @return  Lead time seconds, BAD_INDEX for none
   */
  int ithLeadSeconds(const time_t gt, const int i) const;
  ...
  /**
   * @return The index of the lead time that corresponds to input
   *         generation time and lead seconds, BAD_INDEX for none
   *
   * @param[in] lt  Lead time seconds
   */
  int leadSecondsIndex(const time_t gt, const int lt) const;
  ...
  /**
   * @return  All lead hours at this generation time
   */
  std::vector<double> getLtHours(const time_t gt) const;
  ...
  /**
   * @return  All lead seconds at this generation time
   */
  std::vector<int> getLtSeconds(const time_t gt) const;

  /**
   * @return  All lead seconds. This method takes into account intra hour
   *          generation times with unique sets of lead times.
   */
  std::vector<int> getAllLtSeconds(void) const;
  ...
  inline double ithFcstHour(const int i) const
  {
    ...
    {
       return fmin(ithLeadHours(i), pLtHour1);
    }
  }
  ...
  int getIntraHourGenIndex( const time_t) const;
 
  /**
   * Static function to set the seconds based on hours so that it works 
   * for hours not representable exactly as floats
   *
   * @param[in] hours
   * @param[out] seconds
   *
   * @note  Rounds to nearest good value when rounding is needed
   * @note Assumes seconds*X = 3600 where X is an integer
   */
   static void convertToSeconds(double hours,  int &seconds);
    ...
    *
    * @note  Rounds to nearest good value when rounding is needed
    * @note Assumes seconds*X*N = 3600 where X is an integer, and N=1,2,..
   */
   static void convertToSecondsWithinHour(double hours, int &seconds);
  ...
  bool pUniqueIntraHourLeadSets; /**< True if intra hour forecasts have 
                                   * unique sets of lead times > */
  int pNumIntraHourGts; /**< Number of intra hour generation times> */
  int pLtSeconds0;    /**< earliest lead seconds, derived from pLtHour0 */
  int pLtSeconds1;    /**< latest lead seconds, derived from pLtHour1 */
  int pLtDeltaSeconds;  /**< lead delta seconds, derived from pLtDeltaHours */
...
private:  
  ...
  bool pIsPeriodic(void) const;
   ...
   void pCheckLtConsistency(void) const;
