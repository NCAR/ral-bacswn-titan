...
public:
  ...
  /**
   * Return string representation of a forecast time that is human friendly,
   * and shows generation and lead time
   *
   * @param[in] ut  Generation time
   * @param[in] lt   Lead time seconds
   *
   * @return  The string
   *
   * @note  This implementation returns "yyyy/mm/dd hh:mm:ss+ssssssss"
   * (8 digits for lead seconds)
   */
  static std::string ftime(const time_t &ut, int lt);
