...
public:
   ...
   * @param[in] leadSecondsRes  Resolution of lead times of of 
   *                         climatological forecast data distributions 
   * @param[in] maxLeadSeconds  Maximum expected lead time seconds
   ...
   * for each sub-grid and at generation time and lead time resolutions.
   *
   * An alternate initialization in which lead times are unevenly spaced
   *
   * @param[in] numSubgrids  Number of sub-grids of domain
   * @param[in] genHourRes  Resolution of generation times of 
   * @param[in] leads       The unevenly spaced leads
   * @param[in] inputDir  Directory containing climatological data 
   *                      distributions
   * @param[in] obsBias  Vector of multipliers to apply to climatological 
   *                     histograms of observation data
...
private:
  ...
  /**
   * Alternate representation of leads when they are unevenly spaced.
   * replaces pNumLeads and pLeadHourRes/pLeadSecondsRes, unless empty
   */
  UnevenLead pUnevenLeads;
  ...
  void pComputeLinearFunction(const std::vector <float> &fcstHist, 
   ...
   int pLeadIndex(float leadHour) const;

   int pLeadSeconds(int leadIndex) const;
};
