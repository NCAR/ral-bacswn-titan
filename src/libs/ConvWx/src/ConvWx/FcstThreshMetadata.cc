			     ...
			     const std::vector<double> &threshold)
{
  ...
  {
    LOG(ERROR) <<  "Inconsistent number of tiles " << threshold.size() << " " << pNumTiles;
    return;
  } 
...
void FcstThreshMetadata::merge(const FcstThreshMetadata &info)
{
  ...
  {
    LOG(ERROR) <<  "Inconsistent number of tiles " << info.pNumTiles << " " << pNumTiles;
    return;
  } 
...
FcstThreshMetadata::nameThreshAverage(int tileIndex) const
{
  ...
  for (size_t i=0; i<names.size(); ++i)
  {
    ...
    {
      LOG(ERROR) << "No matching names to " << name << " for tile " << tileIndex;
    }
  }
  return ret;
}
