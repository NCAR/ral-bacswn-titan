...
bool DynamicWeightsHandler::getWeight(int ipos, double &weight) const
{
  ...
  {
    LOG(DEBUG_VERBOSE) << "have no data ipos=" << ipos;
    return false;
  }
}
...
bool DynamicWeightsHandler::getMaxWeight(int ipos, double &weight) const
{
  ...
  {
    LOG(ERROR) << "have no data ipos=" << ipos;
    return false;
  }
}
...
bool DynamicWeightsHandler::getThresh(int ipos, double &thresh) const
{
  ...
  {
    LOG(ERROR) << "No thresholds allowed";
    return false;
  }
}
...
bool DynamicWeightsHandler::maxGrid(Grid &ret) const
{
  ...
  {
    LOG(ERROR) << "No grids to take max of";
    return false;
  }
}
				 ...
				 const FcstGrid &grid, double &weight) const
{
  if (interpWeight == 0.0)
  {
    LOG(DEBUG_VERBOSE) <<" The only available data has interpolating weight=0";
    weight = 0.0;
    return false;
  }
  else
...
bool DynamicWeightsHandler::pMin(int ipos, double &weight) const
{
  ...
  {
    LOG(ERROR) << "Not both data values at ipos=" << ipos;
    weight = 0;
    stat = false;
  }
  else if (ok1 && !ok0)
  ...
  {
    LOG(ERROR) << "Have no data at ipos=" << ipos;
    weight = 0.0;
    stat = false;
  }
  return stat;
