  ...
  pEarthRadiusKm(proj.pEarthRadiusKm)
{
  if (pNx <= 0 || pNy <= 0)
  {
    LOG(ERROR) << "Grid shrunk too much took off " << nx << ","  << ny
	       << " result nx,ny=" << pNx << "," << pNy;
  }
}
...
bool ParmProjection::consistentForTiling(const ParmProjection &proj) const
{
  ...
  {
    LOG(WARNING) << "projections differ in dx,dy "
		 <<  proj.pDx << "," <<  proj.pDy<< " vs "
		 << pDx << "," << pDy;
    ret = false;
  }
  if (proj.pProjection != pProjection)
  {
    LOG(WARNING) << "projections differ in type "
		 << proj.pProjection << " vs " << pProjection;
    ret = false;
  }
  return ret;
}
...
bool ParmProjection::isSuperset(const ParmProjection &proj) const
{
  ...
  {
    LOG(WARNING) << "superset check failed";
    ret = false;
  }
  ...
  {
    char buf[1000];
    sprintf(buf, "[%lf,%lf],[%lf,%lf] not a superset of [%lf,%lf],[%lf,%lf]",
