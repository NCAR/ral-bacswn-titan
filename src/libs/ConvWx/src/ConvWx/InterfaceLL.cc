				...
				time_t &gt, int &lt)
{
  ...
  {
    LOG(ERROR) << "string not a path " << input;
    return false;
  }
  ...
  {
    LOG(ERROR) << "scanning for yyyymmdd '" << ymd << "'";
    return false;
  }
  if (sscanf(hms.c_str(), "%02d%02d%02d", &h, &min, &sec) != 3)
  {
    LOG(ERROR) << "scanning for hhmmss '" << hms << "'";
    return false;
  }
  if (sscanf(seconds.c_str(), "%d.mdv", &lt) != 1)
  {
    LOG(ERROR) << "scanning for lead seconds '" << seconds << "'";
    return false;
  }
				...
				GridData &edge)
{
  ...
  LOG(DEBUG_VERBOSE) << "START Adding lat/lon";
  vector<pair<double,double> >::const_iterator i;
  for (i=latlon.begin(); i!=latlon.end(); ++i)
  {
    int x, y;
    LOG(DEBUG_VERBOSE) << "Adding lat/lon " << i->first << "," << i->second;
    proj.latlonToGridIndexAny(i->first, i->second, x, y);
    edges.add(x, y);
  }
  if (edges.bad())
  {
    LOG(ERROR) << "constructing the edges";
    return false;
  }
				     ...
				     bool last)
{
  ...
  else
  {
    ...
    {
      LOG(ERROR) << "Could not open file for appending " << filename;
      return false;
    }
  }
  ...
  if (last)
  {
    ...
    {
      LOG(ERROR) << "Could not rename " << filename << " to " <<  finalFilename;
      return false;
    }      
    else
    {
