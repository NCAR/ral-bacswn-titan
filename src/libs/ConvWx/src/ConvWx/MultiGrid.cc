				    ...
				    const bool debugWarn) const
{
  ...
  if (ind == convWx::BAD_INDEX)
  {
    if (debugWarn)
    {
      LOG(ERROR) << "no matching field name " << fieldname;
    }
    return NULL;
  }
  else
...
Grid *MultiGrid::gridPtr(const string &fieldname, const bool debugWarn)
{
  ...
  if (ind == convWx::BAD_INDEX)
  {
    if (debugWarn)
    {
      LOG(ERROR) << "no matching field name s" << fieldname;
    }
    return NULL;
  }
  else
			    ...
			    const string &fieldTo)
{
  ...
  {
    LOG(ERROR) << "matching field names " << fieldFrom << " " << fieldTo;
    return false;
  }
...
void MultiGrid::monotonicDecreasing(const MultiGrid &g)
{
  if (!fieldsMatch(g))
  {
    LOG(ERROR) << "Fields not one to one";
    return;
  }
  for (size_t i=0; i<size(); ++i)
  {
...
void MultiGrid::push_back(const Grid &grid)
{
  ...
  {
    LOG(WARNING) << "Cannot push grid, already there" << grid.getName();
  }
}
