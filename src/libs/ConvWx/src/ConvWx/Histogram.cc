...
void Histogram::binDataOrderZeroVals(const Grid &dataGrid, const Grid &distThrGrid,  
                                     ...
                                     int &numMissing, int &numOverThresh)const
{
  ...
  vector < list < IndexGridValDistVal* > > tmpHist;
  ...
  for ( int i = 0; i < dataGrid.getNdata(); i++)
  {
    ...
    else
    {
      ...
      if (! distThrGrid.getValue(i,dist))
         dist = 1000;
      if (gridVal > pValThresh)
      {
        overThresh++;
      ...
      for (int j = 0; j < numBins; j++)
      {
        ...
        {
          //
          // Count the non zero non missing neighbors
          //  
          int x = dataGrid.xAtIndex(i);
          int y = dataGrid.yAtIndex(i);
          int count = 0;
          dataGrid.nearbyNonMissingGTZero( x,y,1,1,count); 

          IndexGridValDistVal *indexGridValDistVal = new IndexGridValDistVal(i, gridVal, dist, count);

          tmpHist[j].push_back(indexGridValDistVal);
          j = numBins;
        }
      }
    }// end if
  ...
  // Sort Histogram lists in ascending order, copy lists to vectors 
  // 
  list<IndexGridValDistVal*>::iterator iter;

  for (int i = 0; i < numBins; i++)
  {   
    IndexGridValDistPtrCmp indexGridValDistPtrCmp;

    (tmpHist[i]).sort(indexGridValDistPtrCmp);
