...
{
  char buf[1000];
  sprintf(buf, "Clumped reg:(thrsh:%d,%.2f), nextr:(%.2f a:%.2f):",
...
bool Nbrhd::pBuildClump(const Grid &dataGrid, int ix, int iy)
{
  ...
  {
    // build from here out.
    LOG(DEBUG_VERBOSE) << "grow recursive";
    pGrowRecursive(dataGrid, ix, iy);
    LOG(DEBUG_VERBOSE) << "done grow recursive";
    if (pValues.currentClumpArea() >= pMaxClumpPts)
    {
      LOG(DEBUG_VERBOSE) <<  "CLUMP " << pValues.dCurrentClumpValue()
			 << " did overflow " << pValues.currentClumpArea()
			 << "points";
    }
    else
    {
      LOG(DEBUG_VERBOSE) <<  "CLUMP " << pValues.dCurrentClumpValue()
			 << " normal  " << pValues.currentClumpArea()
			 << "points";
    }
    return true;
  }
...
void Nbrhd::pDoMerge(const vector<int> &clumps, const int minClumpNumPoints)
{
  ...
  if (minv == 0.0)
  {
    ...
    if (clumps.size() != 1)
    {
      LOG(ERROR) <<  "clump=0, with " << clumps.size() << " matches";
    }
    return;
  }
