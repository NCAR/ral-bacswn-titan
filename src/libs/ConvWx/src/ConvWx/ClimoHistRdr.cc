...
void ClimoHistRdr::init(const int numSubgrids, const float genHourRes,
			const int leadSecondsRes, const int maxLeadSeconds,
			...
			const int maxAge, const bool realtime, const bool isQuantileData)
{
  ...
  pLeadHourRes = static_cast<float>(leadSecondsRes)/convWx::SECS_PER_HOUR;
  pLeadSecondsRes = leadSecondsRes;
  
  pNumGenHours =
    static_cast<int>(static_cast<float>(convWx::HOURS_PER_DAY)/genHourRes);
  
  pNumLeads = maxLeadSeconds/leadSecondsRes + 1;
  ...
  for (int i = 0; i < pNumGenHours; i++)
  {
	...
	for(int k = 0; k < static_cast<int>(distStats.size()); k++)
	{
	  ...
	  if(!pIsQuantileData)
	  {
	    ...
	    {
	      LOG(ERROR) << "Histograms from " << filepath << " for subgrid " << subgridNum << " do not have the "
			 << "required number of histogram bins. Required is " << pRequiredBinNum << " bins."
			 << "Observation histogram has " << obsHist.size() << " and forecast histogram has "
			 << fcstHist.size() << " bins";
	      continue;
	    }
	  ...
	  else
	  {
		...
		static_cast<int>(fcstHistStats.size())  != pRequiredBinNum)
	    {
	      LOG(ERROR) << "Histograms from " << filepath << " for subgrid " << subgridNum << " do not have the "
			 << "required number of histogram bins. Required is " << pRequiredBinNum << " bins."
			 << "Observation histogram has " << obsHistStats.size() << " and forecast histogram has "
			 << fcstHistStats.size() << " bins";
	      continue;
	    }
				  ...
				  vector <float> &ratio)
{
  ...
  if ( static_cast<int>(obsHist.size()) != pRequiredBinNum)
  {
    LOG(DEBUG_VERBOSE) << "check size of climo shift bias array in param file, bias not applied!";
  }
  else
  {
    for (int i = 0; i < pRequiredBinNum; i++)
    {
      LOG(DEBUG_VERBOSE) << "before bias (" << pObsBias[i] << "):obsHist[" << i << "]:" << obsHist[i];

      obsB[i] = static_cast<int>(static_cast<float>(obsHist[i]) * pObsBias[i]);

      LOG(DEBUG_VERBOSE) << "after bias  obsHist[" << i << "]: " << obsB[i]; 
    }
  }
  ...
  { 
    ...
    {
      // 
      // assume a fcst histogram bin with 1 element.
      //
      rat = obsB[i];
    }
						      ...
						      float leadHour) const
{
  ...
  //
  // Integer representing the lead hour
  //
  int leadNum = pLeadIndex(leadHour);
  
  LOG(DEBUG_VERBOSE) << "Using subg " << subgridNum << " genHour " << genNum*pGenHourRes
		     << " leadHour " << leadHour << " genNum " << genNum
		     << " leadNum " << leadNum << " fracShifts size "
		     <<  pRatiosObsModel[subgridNum][genNum][leadNum].size();
							    ...
							    float leadHour) const
{
  ...
  LOG(DEBUG_VERBOSE) << "Using subg " << subgridNum << " genHour " << genNum*pGenHourRes
                     << " leadHour " << leadHour << " genNum " << genNum
		     << " leadNum " << leadNum << " slopes size "
		     <<  pSlopes[subgridNum][genNum][leadNum].size();
								...
								float leadHour) const
{
  ...
  LOG(DEBUG_VERBOSE) << "Using subg " << subgridNum << " genHour " << genNum*pGenHourRes
		     << " leadHour " << leadHour << " genNum " << genNum
		     << " leadNum " << leadNum << " intercepts size "
		     <<  pIntercepts[subgridNum][genNum][leadNum].size();
...
int ClimoHistRdr::pLeadIndex(float leadHour) const
{
  //
  // Integer representing the lead time 
  //
  int leadSeconds, leadNum;
  ParmFcst::convertToSeconds(leadHour, leadSeconds);

  if (pUnevenLeads.isSet())
  {
     leadNum = pUnevenLeads.leadIndex(leadSeconds);
     if (leadNum == convWx::BAD_INDEX)
     {
        LOG(ERROR) << "Unconfigured for lead seconds " << leadSeconds;
        exit(1);
     }
  }
  else
  {
     ...
     leadNum = leadSeconds/pLeadSecondsRes;
  }
  return leadNum;
}

int ClimoHistRdr::pLeadSeconds(int leadIndex) const
{
  int sec;
  if (pUnevenLeads.isSet())
  {
     sec  = pUnevenLeads[leadIndex];
  }
  else
  {
     //int leadNum = static_cast<int>(leadHour/pLeadHourRes);
     sec = static_cast<int>(leadIndex*pLeadHourRes*convWx::SECS_PER_HOUR);
  }
  return sec;
}
