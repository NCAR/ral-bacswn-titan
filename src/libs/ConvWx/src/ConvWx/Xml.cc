			       ...
			       vector<pair<string,string> > &pairs)
{
  ...
  {
    LOG(ERROR) << "reading tagbuf array";
    return false;
  }
  ...
  for (int i=0; i<static_cast<int>(elem.size()); ++i)
  {
    ...
    else
    {
      ...
      {
	LOG(SEVERE) << i <<"th element= '" << elem[i] << "' return='" << s << "'";
	if (printedBad == false)
	{
	  printedBad = true;
	  LOG(SEVERE) << "WHERE I DONT WANT TO BE";
	  LOG(SEVERE) << "The entire input looks like this:";
	  LOG(FORCE) << buf;
	}
      }
      if (a.empty())
	...
	{
	  LOG(WARNING) << "attributes but not "  << attribute << " as expected '" << elem[i] << "'";
	  pair<string,string> p(s, "");
	  pairs.push_back(p);
	}
	else
		     ...
		     size_t searchStart, size_t *searchEnd)
{
  ...
  if (!pFindTagLimits(xmlBuf, tag, searchStart, startPos, endPos))
  {
    if (doPrintErrorMsg)
    {
      LOG(ERROR) << "finding tag limits for tag=" << tag;
    }
    return false;
  }
		     ...
		     vector<Attribute> &attributes)
{
  ...
  else
  {
    ...
    if (endPos == string::npos)
    {
      if (doPrintErrorMsg)
      {
	LOG(ERROR) << "finding endtag " << endTok;
      }
      return false;
    }
		  ...
		  const bool doPrintErrorMsg, int &val)
{
  ...
  if (!pReadInt(valStr, val))
  {
    if (doPrintErrorMsg)
    {
      LOG(ERROR) << "interpreting as int:" << valStr;
    }
    return false;
  }
  return true;
		     ...
		     const bool doPrintErrorMsg, double &val)
{
  ...
  if (!pReadDouble(valStr, val))
  {
    if (doPrintErrorMsg)
    {
      LOG(ERROR) << "interpreting as double:" <<  valStr;
    }
    return false;
  }
  return true;
		      ...
		      const bool doPrintErrorMsg, bool &val)
{
  ...
  if (!pReadBoolean(valStr, val))
  {
    if (doPrintErrorMsg)
    {
      LOG(ERROR) << "interpreting as bool:" <<  valStr;
    }
    return false;
  }
  return true;
		   ...
		   const bool doPrintErrorMsg, time_t &val)
{
  ...
  if (!readTime(valStr, val))
  {
    if (doPrintErrorMsg)
    {
      LOG(ERROR) << "interpreting as time:" <<  valStr;
    }
    return false;
  }
  return true;
...
bool Xml::readTime(const string &valStr, time_t &val)
{
  ...
  LOG(ERROR) << "Cannot decode string as time:" << valStr;
  return false;
}
			 ...
			 string &val)
{
  ...
  if (doPrintErrorMsg)
  {
    LOG(WARNING) << "didn't find attribute " << name;
  }
  return false;
}
  ...
  
{
  ...
  {
    LOG(ERROR) << "Cannot decode string into int: " << valStr;
    return false;
  }
  val = ival;
  return true;
  ...
  
{
  ...
  {
    LOG(ERROR) << "Cannot decode string into bool: " << valStr;
    return false;
  }
  return true;
}
...
bool Xml::pReadDouble(const string &valStr, double &val)
{
  ...
  {
    LOG(ERROR) << "Cannot decode string into double:: " << valStr;
    return false;
  }
  val = dval;
  return true;
