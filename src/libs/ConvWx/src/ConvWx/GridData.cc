  ...
  pMissing(bad)
{
  ...
  for (size_t i=0; i<data.size(); ++i)
  {
    ...
    {
      LOG(WARNING) << "Out of range " << i << "  [0," << pNptTotal << "] ignored";
    }
    else
    {
      pDataPtr[i] = data[i];
...
bool GridData::copyValues(const GridData &g)
{
  ...
  {
    LOG(ERROR) << "dims unequal "
	       << pNptX << "," << pNptY << "," << pNptZ
	       << "  "
	       << g.pNptX << "," << g.pNptY << "," << g.pNptZ;
    return false;
  }
  for (int i=0; i<pNptTotal; ++i)
  {
...
bool GridData::copyFloat(float *data, const int npt) const
{
  if (npt != pNptTotal)
  {
    LOG(ERROR) << "npt:" << npt << " pNptTotal:" << pNptTotal;
    return false;
  }
  for (int i=0; i<pNptTotal; ++i)
  {
				   ...
				   const int npt) const
{
  if (npt != pNptTotal)
  {
    LOG(ERROR) << "Field " << fieldName << " npt:" << npt << " pNptTotal:" << pNptTotal;
    return false;
  }
  int nnan = 0;
  for (int i=0; i<pNptTotal; ++i)
  {
    if (!std::isfinite(pDataPtr[i]))
    {
      ++nnan;
      LOG(DEBUG_VERBOSE) << fieldName << "[" << i << "] = NaN, replace with missing";
  ...
  if (nnan > 0)
  {
    LOG(DEBUG) << fieldName << " replaced " << nnan << " NaN values with missing";
  }
  return true;
}
			  ...
			  bool yWrap, double &value, bool &outOfBounds) const
{
  if (x0 < 0 || x0+nx > pNptX)
  {
    if (!xWrap)
    {
      LOG(ERROR) << "subset out of grid range X [" << x0 << "," << x0+nx-1 << "]  [0," << pNptX-1 << "]";
      outOfBounds = true;
      return false;
    }
  }
  if (y0 < 0 || y0+ny > pNptY)
  {
    if (!yWrap)
    {
      LOG(ERROR) << "subset out of grid range Y [" << y0 << "," << y0+ny-1 << "]  [0," << pNptY-1 << "]";
      outOfBounds = true;
      return false;
    }
  }
...
void GridData::changeValue(const double vold, const double vnew)
{
  for (int i=0; i<pNptTotal; ++i)
  {
    if (fabs( pDataPtr[i] - vold) < convWx::EPSILON)
    {
      pDataPtr[i] = vnew;
    }
  }
			    ...
			    const double excludeValue)
{
  ...
  if (!sizeEqual(mask))
  {
    LOG(ERROR) << "npt:" << mask.pNptTotal << " pNptTotal:" << pNptTotal;
    return false;
  }
...
void GridData::nearbyNonMissingGTZero( const int centerX, const int centerY, const int sx,
                             const int sy, int &count) const
{  
  count=0;
...
bool GridData::reduce(const int f)
{
  if (f < 2)
  {
    LOG(WARNING) << "cant reduce grid factor is too small " << f;
    return false;
  }
  int nx = pNptX/f;
  int ny = pNptY/f;
  if (nx < 1 || ny < 1)
  {
    LOG(WARNING) << "cant reduce grid factor is too big " << f;
    return false;
  }
...
bool GridData::interpolate(const GridData &lowres, const int res)
{
  ...
  {
    LOG(ERROR) << "lowrespNptX != pNptX/res";
    return false;
  }
  if (lowres.pNptY != pNptY/res)
  {
    LOG(ERROR) << "lowrespNptY != pNptY/res";
    return false;
  }
...
{
  LOG(DEBUG_VERBOSE) << "replacing " << oldv << " with " << newv;
  int count = 0;
  for (int i=0; i<pNptTotal; ++i)
  {
    if (pDataPtr[i] == oldv)
...
//----------------------------------------------------------------
int GridData::replaceBelowThresh(double floorValue,
				 double replacementValue)
{
  LOG(DEBUG_VERBOSE) << "replacing when below" << floorValue << " with "
		     << replacementValue;
  int count = 0;
  for (int i=0; i<pNptTotal; ++i)
  {
    if (pDataPtr[i] < floorValue)
    {
      ++count;
      pDataPtr[i] = replacementValue;
    }
  }
  return count;
}
...
void GridData::replaceValueWithNearestMax(const double value)
{
  ...
  for (int y=0; y<pNptY; ++y)
  {
	...
	LOG(DEBUG_VERBOSE) << "replacing at " << x << "," << y;
		    ...
		    GridData &outGrid) const
{
      ...
      ||  pNptZ != outGrid.pNptZ)
  {
    LOG(ERROR) << "dims unequal "
	       << pNptX << "," << pNptY << "," << pNptZ
	       << "  "
	       << outGrid.pNptX << "," << outGrid.pNptY << "," << outGrid.pNptZ;
    return false;
  }
					  ...
					  bool &gotV, double &maxV) const
{
  ...
  LOG(DEBUG_VERBOSE) << "checking " << x << "," << y;
