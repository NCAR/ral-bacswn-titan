...
bool GridAverage::accumulate(const MultiGrid &grid, const Grid &weights)
{
  if (!pWeights)
  {
    LOG(ERROR) << "Weights info not set";
    return false;
  }
...
bool GridAverage::average(const int minFcsts, MultiGrid &mg)
{
  ...
  if (ave)
  {
    LOG(DEBUG) << "Taking average of "<< pNumSum << " things for output";
    ret = true;
  }
  else
  {
    LOG(WARNING) << pNumSum << " fields, not enough. Set all output missing";
    ret = false;
  }
				  ...
				  Grid &wtg, bool debug)
{
  ...
  if (vgt.empty())
  {
    LOG(WARNING) <<"No Weights data within tolerance of trigger time";
    return false;
  }
  ...
  if (input.loadFcstData(wt, lt, proj, fg))
  {
    if (debug)
    {
      LOG(DEBUG) << "Weights data loaded, "
		 << ConvWxTime::stime(wt) << "+" << lt;
    }
    wtg = fg;
    return true;
  }
					 ...
					 time_t &weightsGt)
{
  ...
  LOG(DEBUG) << "Weights gen time set to " << ConvWxTime::stime(weightsGt);
  return true;
...
double GridAverage::pDetermineWeight(const int i, const Grid &weight) const
{
  ...
  if (n == pNumGrids)
  {
    ...
    {
      LOG(ERROR) << "Expected all weights at " << i;
      return -1.0;
    }
  }
  else if (n > 0 && n < pNumGrids)
  {
	...
	return w;
      ...
      {
	LOG(ERROR)<< "Expected non-missing average at " << i;
	return -1.0;
      }
    }
  }
