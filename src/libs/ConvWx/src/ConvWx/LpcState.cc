...
{  
  ...
  {
    int lt = inputModelParms.ithLeadSeconds(modelGt, i);
    if (lt != convWx::BAD_INDEX)
    {
  ...
  {
    int lt = outputModelParms.ithLeadSeconds(outputGt,i);
    if (lt != convWx::BAD_INDEX)
    {
...
void LpcState::print(void) const
{
  ...
  {
    printf("%d %s\n", pModel[i].first, pModel[i].second.sprint().c_str());
  }
  for (int i=0; i<static_cast<int>(pOutput.size()); ++i)
  {
    printf("%d %s\n", pOutput[i].first, pOutput[i].second.sprint().c_str());
  }
}
