...
ConvWxFuzzy::ConvWxFuzzy(const vector<pair<double,double> > &f) : pMapping(f)
{
  ...
  {
    LOG(ERROR) << "succesive input x values cannot be equal, " << pMapping[1].first;
    bad = true;
  }
  ...
  for (size_t i=2; i<pMapping.size(); ++i)
  {
    ...
    {
      LOG(ERROR) << "succesive input x values cannot be equal, " << pMapping[i].first;
      bad = true;
    }
...
{
  LOG(FORCE) << "fuzzy function:";
  for (int i=0; i<static_cast<int>(pMapping.size()); ++i)
  {
    char buf[1000];
    sprintf(buf, "f[%d]=(%12.7lf,%12.7lf)", i, pMapping[i].first,
	    pMapping[i].second);
    LOG(FORCE) << buf;
  }
}
