					 ...
					 const double minPcnt) const
{
  for (int i=0; i<static_cast<int>(pGrids.size()); ++i)
  {
    ...
    {
      LOG(DEBUG_VERBOSE) <<  "Fractional test indicated no p.c. for box at "
			 << x0 << "," << y0;
      return false;
    }
  }
  LOG(DEBUG_VERBOSE) <<  "Fractional test indicate perform P.C. for box at "
			 << x0 << "," << y0;
  return true;
}
				     ...
				     const double value) const
{
  if (pGrids.empty())
  {
    LOG(ERROR) << "no grids for template";
