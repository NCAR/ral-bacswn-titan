...
//------------------------------------------------------------------
string ConvWxTime::ftime(const time_t &ut, int lt)
{
  DateTime_t mtime;
  mtime.unixTime = ut;
  pConvertFromUtime(mtime);
  char str[convWx::ARRAY_LEN_VERY_SHORT];
  sprintf(str, "%.4d/%.2d/%.2d %.2d:%.2d:%.2d+%.8d",
	  mtime.year, mtime.month, mtime.day,
	  mtime.hour, mtime.min, mtime.sec, lt);
  string ret = str;
  return ret;
}  
...
time_t ConvWxTime::utime(const string &stime)
{
  ...
  {
    LOG(ERROR) << "Parsing time value " << stime;
    return 0;
  }
}
					   ...
					   const time_t &lastRun)
{
  ...
  do
  {
    ...
    if (min%10 == 0)
    {
      ...
      LOG(DEBUG) << "Previous run_day " << lastRunDay << ", current day " << day
		 << " current hour " << hour << " (run hour is " << wantedHour << ")";
    }
  }
			     ...
			     const std::string &obsDataUrl)
{
  if (dataTrigger)
  {
    ...
    {
      LOG(WARNING) << "No triggering of " << obsDataUrl << ", use clock time";
      return time(0);
    }
  }
  else
