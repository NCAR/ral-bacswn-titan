		    ...
		    const string &name)
{
  ...
  v = v*delta;
  char buf[1000];
  sprintf(buf, "\t%s = %.2lf km", name.c_str(), v);
...
bool ParmSetUV::storeGridDim(const double dxKm, const double dyKm)
{
  ...
  LOG(DEBUG_VERBOSE) <<  "\tlowResNgridpts=" << pLowResNgridpts;
  ...
  LOG(DEBUG_VERBOSE) << "\tShifting npt(lowres):";
  ...
  LOG(DEBUG_VERBOSE) << "\tRefinement shifting npt(lowres):";
