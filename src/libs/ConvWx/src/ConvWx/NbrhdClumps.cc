...
void NbrhdClumps::merge(const vector<int> &clumps, const int minClumpNumPts)
{
  ...
  {
    LOG(DEBUG_VERBOSE) <<  "clump " << minClumpNumPts << " after merge too small " << count << " points";
    pRemoveClump.push_back(finalClump);
  }
  else
  {
    LOG(DEBUG_VERBOSE) <<  "clump " << minClumpNumPts << " after merge big enough " << count << " points";
  }
}
...
const NbrhdClump *NbrhdClumps::pMatchingClump(const int clumpValue) const
{
  ...
  LOG(ERROR) << "never found " << clumpValue;
  return NULL;
}
			   ...
			   Grid &scaleGrid) const
{
  ...
  if (v != 1.0)
  {
    LOG(DEBUG_VERBOSE) << "reducing clump " << clump.clumpValue() << " by " << v;
