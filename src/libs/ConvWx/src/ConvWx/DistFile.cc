...
{
  LOG(DEBUG) << "Parsing file: " <<inputPath;
  ...
  {
    LOG(WARNING) <<  "Error opening " << inputPath;
    return false;
  }
  else
  {
...
{  
  ...
  for (int i = 0; i < static_cast<int>(aveStats.size()); i++)
  {
    ...
    {
      LOG(DEBUG) << "Writing data to " << outputDir << " for " << ymd << " generation hour " << i;
            ...
            if (static_cast<int>(aveStats[i][j].size()) > 0)
            {
                ...
                for (int k = 0; k < static_cast<int>(aveStats[i][j].size()); k++)
                {
                    ...
                    if (isInt) {
                        ...
                        LOG(DEBUG_VERBOSE) << "Writing data for year month day " << ymd
					   << ", generation hour" << genHour << ", lead time "
					   << leadSecs << ", subgrid " << gridNum
					   << " to " << outputDir;
    ...
    {
      LOG(DEBUG) << "No data written to " << outputDir << " for " << ymd << " generation hour " << i;
    }
