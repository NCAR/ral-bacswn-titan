			 ...
			 ConvWxMultiThreadMgr *threads)
{
  ...
  LOG(DEBUG_VERBOSE) << "Begin computing phase correction vectors";
			    ...
			    const Grid &data, Grid &pc)
{
  ...
  LOG(DEBUG) << "Doing phase correction now";
  pc = data;
...
bool UvOutput::normalize(const Grid &normGrid)
{
  ...
  if (nnx != nx || nny != ny)
  {
    LOG(ERROR) << "norm grid " << nnx << "," << nny << " local:" << nx << "," << ny;
    return false;
  }
			 ...
			 Grid &uGrid, Grid &vGrid) const
{
  ...
  case UvUnits::METERS_PER_SECOND:
    ...
    break;
  default:
    LOG(ERROR) << "bad local units " << pUnits;
  }
}
					...
					int &nx, int &ny) const
{
  ...
  if (cnx != nx || cny != ny)
  {
    LOG(ERROR) << "dims unequal " << cnx << "," << cny << "  " << nx << "," << ny;
    return false;
