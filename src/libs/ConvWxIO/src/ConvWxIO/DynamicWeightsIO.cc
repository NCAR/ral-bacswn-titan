				 ...
				 ConvWxThreadMgr *threads) const
{
  ...
  if (!hasData)
  {
    LOG(WARNING) << "no data at " << parms.pUrl << " time="
  ...
  if (debug)
  {
    LOG(DEBUG_VERBOSE) <<  "data does exist at " << parms.pUrl << " time="
		       << ConvWxTime::ftime(gt, lt);

    LOG(DEBUG) << "Loading data now from " << parms.pUrl << ","
	       << ConvWxTime::ftime(gt, lt);
  }
  ...
  if (!hasData)
  {
    LOG(ERROR) <<  "expected data but none at " << parms.pUrl << " time="
		       << ConvWxTime::ftime(gt, lt);
    return false;
  }
  // if (debug)
  // {
    LOG(DEBUG) << "Did load data from " << parms.pUrl << " time="
	       << ConvWxTime::ftime(gt, lt);
    //}
  if (setMissingToZero)
  {
