  ...
  pErrorLatencyHours(blendingHtml::badLatencyHours)
{
  ...
  {
    LOG(ERROR) << "forming path recursively " << pIndexFile;
    pPath = "";
    return;
  }
  ...
  if (fp == NULL)
  {
    ...
    if (fp == NULL)
    {
      LOG(ERROR) << "opening for write:" << pIndexFile;
      pPath = "";
      return;
    }
    fprintf(fp, s.c_str());
		       ...
		       const string &imissing) const
{
  ...
  {
    LOG(ERROR) << "logging blend into non-blend file";
    return;
  }
		       ...
		       const string &imissing) const
{
  ...
  {
    LOG(ERROR) << "logging blend with mask into non-blend file";
    return;
  }
		       ...
		       const UvHtml &uv) const
{
  ...
  {
    LOG(ERROR) << "logging pc into non-pc file";
    return;
  }
...
void BlendingHtml::pUpdateIndexFile(const BlendingHtmlLine &line) const
{
  ...
  {
    LOG(ERROR) << "finding TABLE key in indx file";
    return;
  }
  if (!Xml::readXmlStringArray(tagbuf, "TR", vv))
  {
    LOG(ERROR) << "reading tag buf array key=TR in index file";
    return;
  }
  ...
  for (int i=0; i<static_cast<int>(vv.size()); ++i)
  {
	...
	gotHeader = true;
      ...
      {
	LOG(WARNING) << "got more than one header " 
		     << tableHeader << " " <<  vv[i];
      }
    }
    else
    {
      LOG(FORCE) << "Unexpected entry in index file " << vv[i];
    }
  }
  ...
  {
    LOG(WARNING) << "dayfile already present " << dayfile;
  }
