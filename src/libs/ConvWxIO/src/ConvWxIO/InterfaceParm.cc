...
#include <ConvWxParams/ParamsNetCdfOut.hh>
...
static bool sMissingRet(const ParmFcst &f, const string &name)
{
  ...
  if (f.pIsRequired)
  {
    LOG(ERROR) << name << ":" << f.pName << " not present in param file but is required";
    return false;
  }
  else
  {
			...
			const std::string &name, bool &stat)
{
  ...
  {
    LOG(ERROR) << "wanted " << f.size() << " for " << name << " got " << n;
    stat = false;
  }
}
...
static bool sSetNewObs(const ParamsObsInOut::obs_data_t &P, ParmFcst &f)
{
  if (strcmp(P.url, "urlNotSet") == 0 || strlen(P.url) == 0)
  {
    LOG(ERROR) << "can't have missing input to variable inputs";
    return false;
  }
  else
  {
					...
					ParmProjection &p)
{
  ...
  {
    LOG(DEBUG) << "Loading file " << parmfile;
    return false;
  }
  if (!P.checkAllSet(stdout))
  {
    LOG(ERROR) << "file has missing parms. All params must be set:" << parmfile;
    return false;
  }
  ...
  case ParamsProjection::REMAP_LAMBERT_AZIM_EQUAL_AREA:
    ...
    break;
  default:
    LOG(ERROR) << "projection in file not in synch with ConvWx code: " << parmfile;
    return false;
  }
			    ...
			    ParmProjection &proj)
{
  ...
  {
    LOG(ERROR) << "Loading file " << sParamPath;
    return false;
  }
  ...
  {
    LOG(ERROR) << "projection file not good:" <<  P.projection_param_file;
    stat = false;
  }
  return stat;
}
			    ...
			    ParmProjection &proj)
{
  ...
  if (!sLoadProjectionParams(P.projection_param_file, proj))
  {
    LOG(ERROR) << "projection file %s not good: " <<  P.projection_param_file;
    stat = false;
  }
  return stat;
}
		    ...
		    bool &stat, ParmPc &p)
{
  ...
  {
    LOG(ERROR) << "No UV input as other, or no vName";
    stat = false;
  }
...
static bool sLoadMainParams(ParmMain &p)
{
  ...
  if (!P.checkAllSet(stdout))
  {
    LOG(ERROR) << "file has missing parms. All params must be set:" << sParamPath;
    return false;
  }
  ...
  {
    LOG(ERROR) << "Loading debug file " << P.debug_file;
    return false;
  }
  if (!D.checkAllSet(stdout))
  {
    LOG(ERROR) <<  "debug file has missing parms. All params must be set:" << P.debug_file;
    return false;
  }

  if (sParamArchive)
  {
    LOG(DEBUG) << "detected archive mode";
  }
...
static bool sLoadMainFcstSubsetParams(ParmMain &p)
{
  ...
  if (sParamArchive)
  {
    LOG(DEBUG_VERBOSE) << "detected archive mode";
  }
...
bool InterfaceParm::loadPcParams(ParmMain &main, ParmPc &p)
{
  ...
  if (P.load(sParamPath.c_str(), NULL, !sParamPrint, false) != 0)
  {
    LOG(ERROR) << "Loading Pc params, file " << sParamPath;
    stat = false;
  }
  if (A.load(sParamPath.c_str(), NULL, !sParamPrint, false) != 0)
  {
    LOG(ERROR) << "Loading Pc Alg params, file " << sParamPath;
    stat = false;
  }
  if (sParamType == PARM_PRINT_AND_LOAD)
  {
  ...
  if (!P.checkAllSet(stdout))
  {
    LOG(ERROR) << "file has missing PC parms. All params must be set:" << sParamPath;
    stat = false;
  }
  if (!A.checkAllSet(stdout))
  {
    LOG(ERROR) << "file has missing PC Algo parms. All params must be set:" << sParamPath;
    stat = false;
  }
...
bool InterfaceParm::loadPcParams(ParmMain &main, ParmPc &p, ParmFcst &obs)
{
  ...
  if (P.load(sParamPath.c_str(), NULL, !sParamPrint, false) != 0)
  {
    LOG(ERROR) << "Loading Pc params from " << sParamPath;
    stat = false;
  }
  if (A.load(sParamPath.c_str(), NULL, !sParamPrint, false) != 0)
  {
    LOG(ERROR) << "Loading Pc Alg params from " << sParamPath;
    stat = false;
  }
  if (sParamType == PARM_PRINT_AND_LOAD)
  {
  ...
  if (!A.checkAllSet(stdout))
  {
    LOG(ERROR) << "file has missing PcAlg parms. All params must be set:" <<  sParamPath;
    stat = false;
  }
...
bool InterfaceParm::loadConvParams(ParmConv &parm)
{
  ...
  {
    LOG(ERROR) << "file has missing convergence parms. All params must be set:" << sParamPath;
    stat = false;
  }
  sSetParmconv(C, parm);
  return stat;
...
bool InterfaceParm::loadSetUvParams(ParmSetUV &p)
{
  ...
  {
    LOG(ERROR) << "Loading SetUv params file " << sParamPath;
    return false;
  }
  if (C.load(sParamPath.c_str(), NULL, !sParamPrint, false) != 0)
  {
    LOG(ERROR) << "Loading Conv params file " << sParamPath;
    return false;
  }
  if (sParamType ==  PARM_PRINT_AND_LOAD)
  {
  ...
  {
    LOG(ERROR) <<  "file has missing parms. All setuv params must be set:" << sParamPath;
    return false;
  }
  if (!C.checkAllSet(stdout))
  {
    LOG(ERROR) <<  "file has missing parms. All convw params must be set:" << sParamPath;
    return false;
  }
...
bool InterfaceParm::loadPcAlgParams(ParmPcAlg &pcParms)
{
  ...
  if (Par.load(sParamPath.c_str(), NULL, !sParamPrint, false) != 0)
  {
    LOG(ERROR) << "Loading ParmPcAlg params file " << sParamPath;
    return false;
  }
  if (sParamType ==  PARM_PRINT_AND_LOAD)
  {
  ...
  if (!Par.checkAllSet(stdout))
  {
    LOG(ERROR) << "file has missing parms. All ParmPcAlg params must be set:" <<  sParamPath;
    return false;
  }
...
//------------------------------------------------------------------
bool InterfaceParm::loadNetCdfParams(const std::string &parmFile,
				     ParmNetCdfOut &p)
{
  ParamsNetCdfOut P;
  if (P.load(parmFile.c_str(), NULL, true, false) != 0)
  ...
  vector<ParmNetCdfOut::NcfData_t> fields;
  for (int i=0; i<P.mdv_ncf_fields_n; ++i)
  {
    ParmNetCdfOut::NcfData_t d;
    d.mdv_field_name = P._mdv_ncf_fields[i].mdv_field_name;
    d.ncf_field_name = P._mdv_ncf_fields[i].ncf_field_name;
    d.ncf_standard_name = P._mdv_ncf_fields[i].ncf_standard_name;
    d.ncf_long_name = P._mdv_ncf_fields[i].ncf_long_name;
    d.ncf_units = P._mdv_ncf_fields[i].ncf_units;
    d.do_linear_transform = P._mdv_ncf_fields[i].do_linear_transform;
    d.linear_multiplier = P._mdv_ncf_fields[i].linear_multiplier;
    d.linear_const = P._mdv_ncf_fields[i].linear_const;
    switch (P._mdv_ncf_fields[i].packed_data_type)
    {
    case ParamsNetCdfOut::DATA_PACK_NONE:
      d.packed_data_type = ParmNetCdfOut::DATA_PACK_NONE;
      break;
    case ParamsNetCdfOut::DATA_PACK_SHORT:
      d.packed_data_type = ParmNetCdfOut::DATA_PACK_SHORT;
      break;
    case ParamsNetCdfOut::DATA_PACK_BYTE:
    default:
      d.packed_data_type = ParmNetCdfOut::DATA_PACK_BYTE;
      break;
    }      
    fields.push_back(d);
  }

  ParmNetCdfOut::NcFormat_t ncf_file_format;
  switch (P.file_format)
  {
  case ParamsNetCdfOut::CLASSIC:
    ncf_file_format = ParmNetCdfOut::CLASSIC;
    break;
  case ParamsNetCdfOut::NC64BIT:
    ncf_file_format = ParmNetCdfOut::NC64BIT;
    break;
  case ParamsNetCdfOut::NETCDF4_CLASSIC:
    ncf_file_format = ParmNetCdfOut::NETCDF4_CLASSIC;
    break;
  case ParamsNetCdfOut::NETCDF4:
  default:
    ncf_file_format = ParmNetCdfOut::NETCDF4;
    break;
  }      
  
  p = ParmNetCdfOut(P.global_attributes.institution,
		    P.global_attributes.references,
		    P.global_attributes.comment,
		    fields,
		    P.compress_data,
		    P.compression_level,
		    P.basename,
		    P.write_to_day_dir,
		    P.output_as_forecast,
		    P.use_iso8601_filename_convention,
		    P.use_TDS_filename_convention,
		    P.use_output_filename,
		    P.output_filename,
		    P.output_file_prefix,
		    P.output_file_suffix,
		    ncf_file_format,
		    P.output_latlon_arrays,
		    P.output_start_end_times,
		    P.output_mdv_attributes,
		    P.output_mdv_chunks,
		    P.force_ldata_as_valid,
		    P.earth_radius_km);
  return true;
}
