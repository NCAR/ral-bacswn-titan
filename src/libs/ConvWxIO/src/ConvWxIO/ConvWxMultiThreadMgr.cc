...
TaThread *ConvWxThreadQue0::clone(const int index)
{
  ...
  {
    LOG(ERROR) << "Cannot clone a thread, pointers not set";
    return NULL;
  }
...

{
  if (sNumThread >= 49)
  {
    LOG(ERROR) << "No more threads, maximum = 50";
    return false;
  }
  ...
  for (int i=0; i<sNumThread; ++i)
  {
    if (sName[i] == name)
    {
      LOG(ERROR) << "Already named thread in uses" << name;
      return false;
    }
  }
  ...
  {
    LOG(ERROR) << "Index out of bounds " << sNumThread;
    return false;
  }
}
				  ...
				  int numThread, bool debug)
{
  ...
  LOG(ERROR) << "No named thread " << name;
  return false;
}
