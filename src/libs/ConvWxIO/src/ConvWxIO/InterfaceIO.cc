...
static bool _commentWasSet = false;
static std::string  _comment = "";

static double sMissingDataValue = -99.0;
static bool sKeepInputMissing = true;

static time_t sCurrentGen = 0;
static time_t sLastCompletedGen = 0;
static std::vector<int> sCurrentLt;

//////////////////////////////////////

static void _checkForComment(DsMdvx &mdvx)
{
...
static string _computeOutputPath(const DsMdvx &mdvx,
				 const std::string &url,
				 const ParmNetCdfOut &nparms)
{
  string ret = "";
  ...
  if(!nparms._output_as_forecast)
  {
    isForecast = false;
  }

  // compute output dir
  string _outputDir = url;
  if (nparms._write_to_day_dir)
  {
    char dayStr[128];
  ...
  {
    LOG(ERROR) << "Cannot make output dir: " << _outputDir;
    return ret;
  }
  ...
  if (nparms._use_output_filename)
  {
    sprintf(outputPath, "%s/%s", _outputDir.c_str(), nparms._output_filename.c_str());
  }
  else if (nparms._use_iso8601_filename_convention)
  {
    if (isForecast)
    { 
      ...
      sprintf(outputPath, "%s/%s%s%.4d-%.2d-%.2dT%.2d:%.2d:%.2d.PT%.2d:%.2d%s.nc",
              _outputDir.c_str(), nparms._output_file_prefix.c_str(),
	      nparms._baseName.c_str(), year, month, day, hour, minute, seconds,
              leadTimeHrs, leadTimeMins, nparms._output_file_suffix.c_str());
    }
    else
    {
      sprintf(outputPath, "%s/%s%s%.4d-%.2d-%.2dT%.2d:%.2d:%.2d%s.nc",
              _outputDir.c_str(), nparms._output_file_prefix.c_str(),
	      nparms._baseName.c_str(), year, month, day, hour, minute, seconds,
	      nparms._output_file_suffix.c_str());
    }
  }
  else if (nparms._use_TDS_filename_convention)
  {
  ...
  {
    string basename = nparms._baseName;
    ...
    { 
      if(basename.empty())
      {
	sprintf(filename, "%sf_%08i%s.nc", nparms._output_file_prefix.c_str(),
		leadTime, nparms._output_file_suffix.c_str());
      }
      else
      {
	sprintf(filename, "%s%sf_%08i%s.nc",
		nparms._output_file_prefix.c_str(), nparms._baseName.c_str(),  
		leadTime, nparms._output_file_suffix.c_str());
      }
      string g_hhmmss = genTime.getTimeStrPlain();
      sprintf(outputPath, "%s/g_%s/%s",  _outputDir.c_str(), g_hhmmss.c_str(), filename);
    }
    else
    {
      if(basename.empty())
      {
	sprintf(filename, "%s%s_%s%s.nc", nparms._output_file_prefix.c_str(),
		v_yyyymmdd.c_str(), v_hhmmss.c_str(), nparms._output_file_suffix.c_str());
      }
      else
      {
	sprintf(filename, "%s%s%s_%s%s.nc", nparms._output_file_prefix.c_str(), nparms._baseName.c_str(),  
		v_yyyymmdd.c_str(), v_hhmmss.c_str(), nparms._output_file_suffix.c_str());
      }
...
static void _writeLdataInfo(const DsMdvx &mdvx, const string &outputPath,
			    ...
			    const ParmNetCdfOut &nparms)
{
  ...
  DsLdataInfo ldata(output_dir, debug);
 
  ldata.setWriter("ConvWxIO");
  ldata.setDataFileExt("nc");
  ldata.setDataType("netCDF");

  string fileName;
  Path::stripDir(output_dir, outputPath, fileName);
  ldata.setRelDataPath(fileName);
  if ( (mhdr.data_collection_type == Mdvx::DATA_EXTRAPOLATED ||
        mhdr.data_collection_type == Mdvx::DATA_FORECAST ||
	mhdr.forecast_time > 0) 
      && !nparms._force_ldata_as_valid)
  {
  ...
  LOG(DEBUG) << "Data written to " << outputPath;
}

static bool _deleteLtriggers()
{
  delete sLtrigger;
  delete sLtrigger2;
  sLtrigger = NULL;
...
static bool _tryToReadLtrigger(int which, int maxTries, const std::string &inputDir,
                               std::string &rpath, LdataInfo &trigger)
{
  string name;
  if (which == 1)
  {
  ...
  {
    LOG(ERROR) << "Bad input which=" << which;
    return false;
  }

  LOG(DEBUG) << "Begin trying to read from " << inputDir;
  int numTries = 0;
  while (trigger.read() == -1)
  {
    ...
    {
      LOG(ERROR) << "Timeout, no new data from " << name << ":" << inputDir;
      return false;
    }
    string msg = "Sleeping Waiting for new ascii input from " + inputDir;
    PMU_auto_register(msg.c_str());
    LOG(DEBUG) << msg;
    sleep(60);
  }
  rpath = trigger.getRelDataPath();
  return true;
}

//------------------------------------------------------------------
static bool sCheckForAllUnevenLeads(const UnevenLead &leads, const time_t &gt, int lt)
{
   if (gt != sCurrentGen)
   {
      if (sCurrentGen != sLastCompletedGen)
      {
         LOG(WARNING) << "Never completed a gen time, never triggered " << ConvWxTime::stime(sCurrentGen);
      }
      sCurrentGen = gt;
      sCurrentLt.clear();
   }
   if (gt == sLastCompletedGen)
   {
      LOG(WARNING) << "Extra lead times with triggering, ignore " << ConvWxTime::ftime(gt, lt);
   }
   else
   {
      sCurrentLt.push_back(lt);
      if (leads.hasAll(sCurrentLt))
      {
         sLastCompletedGen = sCurrentGen;
         return true;
      }
   }
   return false;
...
static void sSetProj(const ParmProjection &p, MdvxProj &proj)
{
  ...
  case ParmProjection::FLAT:
    ...
    break;
  default:
    LOG(ERROR) << "projection is out of synch with ConvWx code";
...
//------------------------------------------------------------------
static void sReadDataIntoGrid(const std::string &field, const std::string &units,
			      fl32 *data, Mdvx::field_header_t &hdr, Grid &gloc)
{
  if (sKeepInputMissing)
  {
    gloc = Grid(field, units, hdr.nx, hdr.ny,  hdr.nz, hdr.missing_data_value);
  ...
  {
    gloc = Grid(field, units, hdr.nx, hdr.ny,  hdr.nz, sMissingDataValue);
    int numSetWrong = 0;
    for (int i=0; i<hdr.nx*hdr.ny*hdr.nz; ++i)
    {
      if (data[i] != hdr.bad_data_value &&
    ...
    if (numSetWrong > 0)
    {
      LOG(WARNING) << numSetWrong
		   << " valid data values have been set to to local missing value "
		   << sMissingDataValue;
    }  
  }
}
...
{
  Grid gr(field, "unknown", p.pNx, p.pNy, p.pNz, sMissingDataValue);
  string path = "missing";
  MetaData metadata;
  ...
  if (sVlevelRestricted)
  {
    ...
    if (vlevels.empty())
    {
      LOG(ERROR) << "Cannot subsample vertical levels, no vlevels";
      return false;
    }
  ...
  if (D.readVolume())
  {
    if (!suppresswarnings)
    {
      LOG(ERROR) << "error reading mdv volume from " << url << " at "
		 << ConvWxTime::ftime(t, lt);
    }
    return false;
  }
  time_t twritten;
  ...
  Grid gloc;
  sReadDataIntoGrid(field, units, data, hdr, gloc);
...
//------------------------------------------------------------------
static bool sLoadByNum(DsMdvx &D, const string &url, const time_t &t,
                       const int lt, int fieldNum, const std::string &field,
		       const bool remap,
                       const ParmProjection &p, FcstGrid &g,
                       bool suppresswarnings)
{
  ...
  // Assume after reading just one field it becomes field 0
  f = D.getFieldByNum(0);
  if (f == NULL)
  {
    if (!suppresswarnings)
		  ...
		  MultiGrid &g, string &path, MetaData &metadata)
{
  ...
  if (D.readVolume())
  {
    ...
    {
      LOG(ERROR) << "reading mdv volume from " << url << " at " <<  ConvWxTime::stime(t);
      for (int i=0; i<static_cast<int>(field.size()); ++i)
      {
	LOG(ERROR) << "reading volume, field name = " << field[i];
      }
    }
    return false;
  }
  ...
  for (int i=0; i<static_cast<int>(field.size()); ++i)
  {
    ...
    if (f == NULL)
    {
      LOG(ERROR) << "reading field " << field[i] << " from " << url
      		 << " at " << DateTime::strn(t);
      gr = Grid(field[i], units, p.pNx, p.pNy,p.pNz, sMissingDataValue);
    }
    else
    {
      f->convertType(Mdvx::ENCODING_FLOAT32, Mdvx::COMPRESSION_NONE);
      ...
      if (hdr.nx != p.pNx || hdr.ny != p.pNy || hdr.nz != expectedNz)
      {
	LOG(WARNING) << "field dimension disagree with expected:" << field[i];
	LOG(WARNING) << "ignore data and create empty grid output:" << field[i];
	gr = Grid(field[i], units, p.pNx, p.pNy,p.pNz, sMissingDataValue);
      }
      else
      {
	...
	else
	{
	  ...
	  if (!(vlevel2 == vlevels))
	  {
	    LOG(WARNING) << firstField << " and " << field[i] << " vert levels are different, use those in " << firstField;
	  }
	}
	fl32 *data = (fl32 *)f->getVol();
	sReadDataIntoGrid(field[i], units, data, hdr, gr);
      }
    }
    g.append(gr);
  }
...
//------------------------------------------------------------------
static void sAddField(DsMdvx &output,
		      const std::vector<std::pair<std::string,std::string> > &shortLongMap,
		      Mdvx::field_header_t &fieldHdr,
		      Mdvx::vlevel_header_t &vlevelHdr, const Grid &o)
{
  string shortName = o.getName();
  string longName = shortName;
  
  for (size_t i=0; i<shortLongMap.size(); ++i)
  {
    if (shortLongMap[i].first == shortName)
    {
      longName = shortLongMap[i].second;
      break;
    }
  }
  fieldHdr.bad_data_value = o.getMissing();
  fieldHdr.missing_data_value = o.getMissing();
  strncpy(fieldHdr.field_name_long, longName.c_str(), MDV_LONG_FIELD_LEN-1);
  fieldHdr.field_name_long[MDV_LONG_FIELD_LEN-1] = '\0';
  strncpy(fieldHdr.field_name, shortName.c_str(), MDV_SHORT_FIELD_LEN-1);
  fieldHdr.field_name[MDV_SHORT_FIELD_LEN-1] = '\0';
  strncpy(fieldHdr.units, o.getUnits().c_str(), MDV_UNITS_LEN-1);
  fieldHdr.units[MDV_UNITS_LEN-1] = '\0';
  MdvxField *f = new MdvxField(fieldHdr, vlevelHdr, (void *)0,true,false);
  fl32 *fo = (fl32 *)f->getVol();
  if (o.copyFloatFilterNans(fo, shortName, fieldHdr.nx*fieldHdr.ny*fieldHdr.nz))
  {
    sConvertType(*f, o);
    output.addField(f);
  }
		     ...
		     int lt, FcstGrid &g)
{
  ...
  if (f == NULL)
  {
    LOG(ERROR) << "reading field 0 from " << url << " at " << DateTime::strn(t);
    return false;
  }
...
static time_t sGetTimeWritten(const time_t &gt, int lt, const ParmFcst &parm)
{
  ...
  if (D.readAllHeaders())
  {
    LOG(ERROR) << "Reading headers from " << parm.pUrl << " at "
	       << ConvWxTime::ftime(gt, lt);
    return false;
  }
			  ...
			  const int maxSeconds)
{
  ...
  if (valstr != NULL)
  {
    LOG_STREAM_TO_LOGFILE(process, instance, "");
...
//------------------------------------------------------------------
bool InterfaceIO::triggerFcst(const std::string &url,
                              const UnevenLead &leads,
                              const bool debug, time_t &triggerGenTime)
{
   // set up sequence triggering
   vector<int> gtSub;
   sTriggeringCheck(url, false, 0, 0, 0, gtSub, true, debug);

   time_t t;
   int lt;
   while (sTrigger->nextTime(t, lt))
   {
      if (sCheckForAllUnevenLeads(leads, t, lt))
      {
         triggerGenTime = t;
         return true;
      }
...
//------------------------------------------------------------------
bool InterfaceIO::triggerFcst(const string &url, 
                              const UnevenLead &leads,
			      const bool debug, const time_t &t0, 
			      const time_t &t1, time_t &t)
{
   vector<int> gtSub;
   sTriggeringCheck(url, false, 0, 0, 0, gtSub, true, t0, t1, debug);
   
   time_t gt;
   int lt;
   while (sTrigger->nextTime(gt, lt))
   {
      if (sCheckForAllUnevenLeads(leads, gt, lt))
      {
         t = sCurrentGen;
         return true;
      }
   ...
   // add some final cleanup here before return?
   return false;
}
			      ...
			      std::string &filepath)
{
  ...
  if(sFileTrigger == NULL)
  {
    ...
    if(sFileTrigger == NULL)
    {
      LOG(ERROR) << "Creation of file trigger failed.";
      return false;
    }
				       ...
				       bool isGenLeadForecastFormat)
{
  ...
  // make triggers as needed
  if (sLtrigger ==NULL)
  {
    LOG(DEBUG) << "Creating file trigger " <<  inputDir1;
    sLtrigger = new LdataInfo(inputDir1, false);
  }
  if (sLtrigger2 ==NULL)
  {
    LOG(DEBUG) << "Creating file trigger " << inputDir2;
    sLtrigger2 = new LdataInfo(inputDir2, false);
  }

  // read from the first input dir, will hang until able to do so,
  // should do so immediately upon restart with existing data
  LOG(DEBUG) << "readBlocking " << inputDir1;
  sLtrigger->readBlocking(-1, 1000, PMU_auto_register);
  rpath1 = sLtrigger->getRelDataPath();
  LOG(DEBUG) << "GOT data from " << inputDir1 << ", filePath1 = " << rpath1;

  // try to read from the second input dir
  if (!_tryToReadLtrigger(2, 20, inputDir2, rpath2, *sLtrigger2))
  {
    // delete the triggers so have a chance to synch on next call
     _deleteLtriggers();
    return false;
  }
  
  filepath1 = inputDir1 + "/" + rpath1;
  filepath2 = inputDir2 + "/" + rpath2;
  LOG(DEBUG) << "GOT filePath1 = " << filepath1;
  LOG(DEBUG) << "GOT filePath2= " << filepath2;

  // want rpath1 = rpath2 or else keep trying
  while (rpath1 != rpath2)
  {
    if (rpath1 < rpath2)
    {
       LOG(DEBUG) << inputDir1 << " older than " << inputDir2 << " look for newer data";

      // this means input1 is older than input2
      // wait for newer inputs from first input dir
      if (!_tryToReadLtrigger(1, 5, inputDir1, rpath1, *sLtrigger))
      {
        // delete the triggers so have a chance to synch on next call
        _deleteLtriggers();
        return false;
    ...
    else // rpath1 must be > rpath2
    {
       LOG(DEBUG) << inputDir2 << " older than " << inputDir1 << " look for newer data";

      // this means input2 is older than input1
      // wait for newer inputs from second input dir
      if (!_tryToReadLtrigger(1, 5, inputDir2, rpath2, *sLtrigger2))
      {
        // delete the triggers so have a chance to synch on next call
        _deleteLtriggers();
        return false;
  ...
  filepath1 = inputDir1 + "/" + rpath1;
  filepath2 = inputDir2 + "/" + rpath2;
  LOG(DEBUG) << "Success, filePath1 = " << filepath1;
  LOG(DEBUG) << "Success, filePath12= " << filepath2;
  return true;
}
...
//------------------------------------------------------------------
std::vector<std::string> InterfaceIO::shortNamesIgnoringEmptyLongNames(const std::string &url,
								       const time_t &genTime,
								       int leadTime)
{
    DsMdvx D;
    D.setReadTime(Mdvx::READ_SPECIFIED_FORECAST, url, 0, genTime, leadTime);
    D.readAllHeaders();
    ...
    for (int i=0; i<nfields; ++i)
    {
      ...
      string sh = D.getFieldHeaderFile(i).field_name;
      if (s.empty())
      {
	LOG(WARNING) << "EMPTY long field name ignored, short field name=" << sh;
      }
      else
      {
	fnames.push_back(sh);
...
vector<int> InterfaceIO::getAvailableLeadTimes(const time_t &gt, 
					       ...
					       const double ltDtH)
{
  LOG(DEBUG_VERBOSE) << "gt: " << ConvWxTime::stime(gt)
		     << ", url:" << url
		     << ", ltHour0:" << ltHour0
		     << ", ltHour1:" << ltHour1
		     << ", ltDtH:" << ltDtH;
  ...
  vector<int> ltimes = InterfaceIO::availableFcstLeadTimeSecondsAtGt(url, gt);
  vector<int> ret;
  LOG(DEBUG_VERBOSE) << "ltimes.size: " << ltimes.size();
  for (int i=0; i<static_cast<int>(ltimes.size()); ++i)
  {
    int lt = ltimes[i];
    ...
    {
      LOG(DEBUG_VERBOSE) << "reject lead time " << lt;
    }
  }
  return ret;
}
...
//------------------------------------------------------------------
vector<int> InterfaceIO::getAvailableLeads(const time_t &gt, 
                                           const string &url,
                                           const int ltSeconds0,
                                           const int ltSeconds1,
                                           const int ltDtS)
{
  LOG(DEBUG_VERBOSE) << "gt: " << ConvWxTime::stime(gt)
		     << ", url:" << url
		     << ", ltSeconds0:" << ltSeconds0
		     << ", ltSeconds1:" << ltSeconds1
		     << ", ltDtS:" << ltDtS;

  double lt0 = static_cast<double>(ltSeconds0);
  double dt = static_cast<double>(ltDtS);
  vector<int> ltimes = InterfaceIO::availableFcstLeadTimeSecondsAtGt(url, gt);
  vector<int> ret;
  LOG(DEBUG_VERBOSE) << "ltimes.size: " << ltimes.size();
  for (int i=0; i<static_cast<int>(ltimes.size()); ++i)
			 ...
			 bool checkTimeWritten, bool useIncomplete)
{
  ...
  if (vgt.empty())
  {
    LOG(WARNING) << "No forecast data back " << maxSecondsBack << " seconds from "
		 << ConvWxTime::stime(t);
    return false;
  }
  ...
  for (int i=ngt-1; i>=0; --i)
  {
	...
	if (checkTimeWritten)
	{
	  ...
	  if (twritten > t)
	  {
	    LOG(DEBUG) << "forecast gen time is too new" <<  ConvWxTime::stime(vgt[i]);
	    good = false;
	  }	    
	}
	if (good)
	{
	  bestT = vgt[i];
	  LOG(DEBUG) << "Best forecast gen time = " << ConvWxTime::stime(bestT);
	  return true;
	}
      }
      else
      {
	LOG(DEBUG) << "Gen time " << ConvWxTime::stime(vgt[i])
		   << " latest lead=" << lt << " not complete";
	if (candidate == 0)
	{
	  candidate = vgt[i];
	}
  ...
  if (useIncomplete)
  {
    ...
    {
      bestT = candidate;
      LOG(WARNING) << "No complete forecast back " << maxSecondsBack << " seconds from "
		   << ConvWxTime::stime(t) << ", using incomplete forecast at "
		   << ConvWxTime::stime(bestT);
      return true;
    }
  }
  LOG(ERROR) << "No complete forecast back " << maxSecondsBack << " seconds from "
	     << ConvWxTime::stime(t);
  return false;
}
					 ...
					 const ParmFcst &parm, time_t &bestT)
{
  ...
  if (vgt.empty())
  {
    LOG(ERROR) << "No forecast data back [" << maxSecondsBack << "," << minSecondsBack
	       << "] seconds from " << ConvWxTime::stime(t);
    return false;
  }
  ...
  for (int i=ngt-1; i>=0; --i)
  {
    ...
    if (!lts.empty())
    {
      ...
      {
	bestT = vgt[i];
	LOG(DEBUG) << "Best forecast gen time = " << ConvWxTime::stime(vgt[i]);
	return true;
      }
      else
      {
	LOG(DEBUG) << "Gen time " << ConvWxTime::stime(vgt[i]) << " latest lead="
		   << lt << " not complete";
      }
    }
  }
  LOG(ERROR) << "No complete forecast back [" << maxSecondsBack << "," << minSecondsBack
	     << "] seconds from " << ConvWxTime::stime(t);
  return false;
}
...
//------------------------------------------------------------------
bool InterfaceIO::loadFcstByNum(const time_t &gt, const int lt, 
				const ParmProjection &p, const string &url,
				...
				const bool remap, FcstGrid &g)
{
  DsMdvx D;
  D.setReadTime(Mdvx::READ_SPECIFIED_FORECAST, url, 0, gt, lt);
  return sLoadByNum(D, url, gt, lt, fieldNum, field, remap, p, g, false);
}

//------------------------------------------------------------------
bool InterfaceIO::loadFcstWithWildcard(const time_t &gt, const int lt, 
                                       ...
                                       const bool remap, FcstGrid &g)
{
  ...
  D.setReadTime(Mdvx::READ_SPECIFIED_FORECAST, url, 0, gt, lt);
  D.readAllHeaders();
  const vector<Mdvx::field_header_t> fh = D.getFieldHeadersFile();
  int index = -1;
  string name;
  for (size_t i=0; i<fh.size(); ++i)
  {
    name = fh[i].field_name;
    if (name.find(wildcardFieldName) != std::string::npos)
    {
       if (index < 0)
       {
          index = (int)i;
       ...
       {
         LOG(ERROR) << "Ambiguous wildcard field name, " << wildcardFieldName
                    << " with more than one match "
                    << name << " " << fh[index].field_name;
         return false;
       }
    }
  }
  if (index < 0)
  {
    LOG(ERROR) << "No fields match wildcard field name, " << wildcardFieldName;
    return false;
  } 
  name = fh[index].field_name;
  return sLoad(D, url, gt, lt, name, remap, p, g, false);
}
			     ...
			     MultiFcstGrid &g)
{
  ...
  if (fnames.empty())
  {
    LOG(ERROR) << "No fields found at "
	       << ConvWxTime::ftime(gt, lt) << " for " << url;
    return false;
  }
  ...
  if (fnames.empty())
  {
    LOG(ERROR) << "No fields found after removes at "
	       << ConvWxTime::ftime(gt, lt) << " for " << url;
    return false;
  }
			...
			const std::vector<std::pair<std::string,std::string> > &shortLongMap)
{
  ...
  for (int i=0; i<o.num(); ++i)
  {
    sAddField(output, shortLongMap, fieldHdr, vlevelHdr, *o.ithConstGrid(i));
  }
...
//------------------------------------------------------------------
void InterfaceIO::writeNetCDF(const time_t &gt, const int lt, const string &url,
			      const ParmProjection &p, const MultiGrid &o,
			      const MetaData &metadata,
			      const ParmNetCdfOut &nparms)
{
  ...
  sOutputInit(gt, lt, p, mdvx, metadata, fieldHdr, vlevelHdr);
  for (size_t i=0; i<nparms._mdv_ncf_fields.size(); ++i)
  {
    const Grid *g = o.constGridPtr(nparms._mdv_ncf_fields[i].mdv_field_name);
    if (g == NULL)
    {
      LOG(ERROR) << "Abort write";
      return;
    }
    sAddField(mdvx, fieldHdr, vlevelHdr, *o.ithConstGrid(i));
  }
  sAddChunk(mdvx, metadata);

  _checkForComment(mdvx);

  if (!_commentWasSet)
  {
    _comment = nparms._comment;
  }
  mdvx.setMdv2NcfAttr(nparms._institution, nparms._references, _comment);
  
  for (size_t ii = 0; ii < nparms._mdv_ncf_fields.size(); ii++)
  {
    DsMdvx::ncf_pack_t packing = DsMdvx::NCF_PACK_ASIS;
    switch (nparms._mdv_ncf_fields[ii].packed_data_type)
    {
    case ParmNetCdfOut::DATA_PACK_NONE:
      packing = DsMdvx::NCF_PACK_FLOAT;
      break;
    case ParmNetCdfOut::DATA_PACK_BYTE:
      packing = DsMdvx::NCF_PACK_BYTE;
      break;
    case ParmNetCdfOut::DATA_PACK_SHORT:
      packing = DsMdvx::NCF_PACK_SHORT;
      break;
    default:
      packing = DsMdvx::NCF_PACK_ASIS;
      break;
    }
    mdvx.addMdv2NcfTrans(nparms._mdv_ncf_fields[ii].mdv_field_name,
                         nparms._mdv_ncf_fields[ii].ncf_field_name,
                         nparms._mdv_ncf_fields[ii].ncf_standard_name,
                         nparms._mdv_ncf_fields[ii].ncf_long_name,
                         nparms._mdv_ncf_fields[ii].ncf_units,
                         nparms._mdv_ncf_fields[ii].do_linear_transform,
                         nparms._mdv_ncf_fields[ii].linear_multiplier,
                         nparms._mdv_ncf_fields[ii].linear_const,
                         packing);
  } // ii
  
  switch (nparms._file_format)
  {
  case ParmNetCdfOut::CLASSIC:
    mdvx.setMdv2NcfFormat(DsMdvx::NCF_FORMAT_CLASSIC);
    break;
  case ParmNetCdfOut::NC64BIT:
    mdvx.setMdv2NcfFormat(DsMdvx::NCF_FORMAT_OFFSET64BITS);
    break;
  case ParmNetCdfOut::NETCDF4_CLASSIC:
    mdvx.setMdv2NcfFormat(DsMdvx::NCF_FORMAT_NETCFD4_CLASSIC);
  break;
  case ParmNetCdfOut::NETCDF4:
  default:
    mdvx.setMdv2NcfFormat(DsMdvx::NCF_FORMAT_NETCDF4);
    break;
  }
  
  mdvx.setMdv2NcfCompression(nparms._compress_data,
                             nparms._compression_level);
  
  mdvx.setMdv2NcfOutput(nparms._output_latlon_arrays,
                        nparms._output_mdv_attributes,
                        nparms._output_mdv_chunks,
                        nparms._output_start_end_times);


  // basic CF
  string outputPath = _computeOutputPath(mdvx, url, nparms);
  if (outputPath.empty())
  {
    return;
  }

  // if the earth radius is set for change, grab the current value now
  double earthRadiusDefault = -1;
  if (nparms._earth_radius_km > 0)
  {
    earthRadiusDefault = TaPjg::getEarthRadius();
    TaPjg::setEarthRadius(nparms._earth_radius_km);
  }
  
  Mdv2NcfTrans trans;
  bool debug = false;
  trans.setDebug(debug);
  // I HOPE THIS IS THE CORRECT CHANGE (Dave Albo)
  //if (trans.translate(mdvx, outputPath))
  if (trans.writeCf(mdvx, outputPath))
  {
    LOG(ERROR) << trans.getErrStr().c_str();
  }
  else
  {
    // write latest data info
    _writeLdataInfo(mdvx, outputPath, url, nparms);
  }

  if (nparms._earth_radius_km > 0)
  {
    // set earth radius back
    TaPjg::setEarthRadius(earthRadiusDefault);
  }
}
...
//------------------------------------------------------------------
void
InterfaceIO::write(const time_t &gt, const int lt, const string &url,
		   const ParmProjection &p, const MultiGrid &o,
		   const MetaData &metadata,
		   const std::vector<std::pair<std::string,std::string> > &shortLongMap,
		   ConvWxThreadMgr &threads)
...
void InterfaceIO::setNoWaitTriggering(int maxWaitSeconds)
{
  ...
  if (sTrigger != NULL)
  {
    LOG(FATAL) << "Setting no wait after creating trigger object";
    exit(-1);
  }
}

//----------------------------------------------------------------
void InterfaceIO::setMissingDataValue(double value)
{
  sMissingDataValue = value;
}

//----------------------------------------------------------------
void InterfaceIO::setKeepInputMissingStatus(bool value)
{
  sKeepInputMissing = value;
}
