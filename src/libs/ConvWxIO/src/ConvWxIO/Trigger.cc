...
bool Trigger::trigger(const ParmMain &parms, time_t &time)
{
  if (sPrintTriggering)
  {
    LOG(PRINT) << "------before trigger------";
  }

  if (sTriggering == Trigger::TRIGGER_OBS_OR_FCST && !sObsOrFcstIsSpecified)
  {
    LOG(ERROR) << "Triggering TRIGGER_OBS_OR_FCST without specifying";
    return false;
  }
  ...
  {
    LOG(ERROR) << "Triggering in wrong mode=" << sprintMode(sTriggering);
    return false;
  }
  if (stat)
  {
    if (sPrintTriggering)
    {
      LOG(PRINT) << "--------Triggered " << ConvWxTime::stime(time) << " ----------";
    }
  }
  else
  {
...
bool Trigger::sequenceTrigger(const ParmMain &parms, time_t &gt, int &lt)
{
  ...
  if (stat)
  {
    if (sPrintTriggering)
    {
      LOG(PRINT) << "--------Triggered " << ConvWxTime::ftime(gt,lt) <<  " ----------";
    }
  }
  else
  {
	...
	while (true)
	{
					 ...
					 dataTimeUrl, t))
	  {
	    if (t - gt < latency)
	    {
	      ...
	      {
		tlast = t;
		LOG(DEBUG) << "obs data " << ConvWxTime::stime(t) << " not " << latency 
			   << " seconds newer than gen time " << ConvWxTime::stime(gt)
			   << "---wait";
	      }
	      sleep(1);
	    }
	    ...
	    {
	      LOG(DEBUG) << "obs data " << ConvWxTime::stime(t) << " at least " << latency 
			   << " seconds newer than gen time " << ConvWxTime::stime(gt)
			   << "---complete trigger now";
	      break;
	    }
	  }
	  else
	  {
	    LOG(WARNING) << "Waiting for any obs data within a day of triggering gen time "
			 << ConvWxTime::stime(gt);
	    InterfaceIO::doRegister("Sleeping");
	    sleep(10);
	  }
...
bool Trigger::genTriggerWLatency(const ParmMain &parms, int latency,
                                  const std::string &dataTimeUrl,
                                  time_t &gt)
  ...
  if (sPrintTriggering)
  {
            ...
            (sTriggering == Trigger::TRIGGER_OBS_OR_FCST && !sTriggeringObs))
   {
     if (parms.pTriggerType == ParmMain::ARCHIVE)
     {
       ...
       // a false status (t0  Lower limit of archive times, t1  Upper limit of archive times)
       stat = InterfaceIO::triggerFcst(parms.pTriggerUrl,
                                       parms.pIsLtSubsample,
                                       parms.pLt0, parms.pLt1,
                                       parms.pDlt,
                                       parms.pGtSubsampleMin,
   ...
   if (stat)
   {
     if (sPrintTriggering)
     {
       LOG(PRINT) << "--------Triggered " << ConvWxTime::stime(gt) << "----------";
     }
   }
   else
   {
...
//------------------------------------------------------------------
bool Trigger::genTriggerWLatencyUnevenLeads(const ParmMain &parms,
                                            ...
                                            time_t &gt)
{
  ...
  if (parms.pTriggerType == ParmMain::ARCHIVE)
  {
     stat = InterfaceIO::triggerFcst(parms.pTriggerUrl, leads, parms.pDebugTrigger,
                                     parms.pArchiveT0, parms.pArchiveT1, gt);
  }
  else
  {
     stat = InterfaceIO::triggerFcst(parms.pTriggerUrl, leads, parms.pDebugTrigger, gt);
  }
  if (stat)
  {
     time_t tlast = 0;
				    ...
				    bool &timedOut)
{
  ...
  {
    LOG(ERROR) << "Triggering in wrong mode=%s" << sprintMode(sTriggering);
    return false;
  }
  ...
  if (!timedOut)
  {
      ...
      {
	LOG(WARNING) << "THE BAD SITUATION";
	LOG(WARNING) << "--------RE-Triggered " << ConvWxTime::ftime(gt, lt) << "----------";
	timedOut = true;
	stat = false;
      }
  }
			      ...
			      string &filepath)
{
  ...
  if (stat)
  {
    if (sPrintTriggering)
    {
      LOG(PRINT) << "--------Triggered interval ["
		 << ConvWxTime::stime(t0) << "," << ConvWxTime::stime(t1)
		 << "]----------";
    }
  }
  else
				      ...
				      string &filePath)
{
  ...
  if (stat)
  {
    ...
    {
      LOG(PRINT) << "--------Triggered interval ["
		 << ConvWxTime::stime(t0) << "," << ConvWxTime::stime(currentT)
		 << "]----------";
    }
  }
  else
  ...
  if (stat)
  {
    if( sPrintTriggering)
    {
       if (isRealtime)
       {
         time_t t1 = time(0);
	 LOG(PRINT) << "--------Triggered interval ["
		    << ConvWxTime::stime(t1-86400) << "," << ConvWxTime::stime(t1)
		 << "]----------";
       }
       else 
       {
...
void Trigger::setTriggeringFcst(const bool isFcst)
{
  ...
  {
    LOG(WARNING) << "specifiying mode with wrong trigger type "
		 << sprintMode(sTriggering);
  }
...
bool Trigger::isForecastTrigger(void)
{
  if (sTriggering != TRIGGER_OBS_OR_FCST)
  {
    LOG(WARNING) << "asking for mode from wrong trigger type "
		 << sprintMode(sTriggering);
    return false;
  }
  else
			    ...
			    const bool allowSubsetTriggering)
{
  ...
  {
    LOG(WARNING) << "subset triggering in mode " << sprintMode(triggerType)
		 << " not allowed, ignore";
