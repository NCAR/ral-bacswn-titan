					     ...
					     bool &allMissing) const
{
  while (true)
  {
    ...
    else
    {
      ...
      {
	LOG(DEBUG) << "waiting " << totalWaitSeconds << " for " << pParm.pUrl << " lt=" << lt;
      }
      else
      {
	// can't wait anymore
	LOG(WARNING) << "no more waiting for " << pParm.pUrl << " lt=" << lt << ", output=all missing";
...
bool BlendingInputData::fillFcstGridWithWait(const time_t &gt, int lt, 
					     ...
					     bool allowLeadZero) const
{
  while (true)
  {
    threads.lock();
    bool hasData;
    if (allowLeadZero && lt == 0)
    {
      hasData = pParm.hasDataIgnoreConfig(gt, lt);
    }
    else
    {
      hasData = pParm.hasData(gt, lt);
    }
				   ...
				   bool &allMissing) const
{
  ...
  {
    LOG(ERROR) << "expected " << pName << " but none " << lt;
    outGrid.setAllMissing();
    allMissing = true;
    return false;
  }
