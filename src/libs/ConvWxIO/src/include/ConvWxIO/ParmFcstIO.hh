...
public:
   ...
   * @param[in] lt  Lead seconds
   *
   * @note queries the server whether gen/lead is configured for or not
   */
  bool hasDataIgnoreConfig(const time_t &gt, const int lt) const;
   ...
   * Load forecast data into a FcstGrid
   *
   * @note same as the other methods, but access data by field  number instead of name
   *
   * @param[in] gt  Generation time
   * @param[in] lt  Lead seconds
   * @param[in] proj   Projection information that may be used 
   * @param[in] fieldNum  Field number: 0, 1, 2, ...
   ...
   * @note queries server
   */
  bool loadFcstDataByNum(const time_t gt, const int lt, const ParmProjection &proj,
                         int fieldNum, const std::string &field, FcstGrid &outGrid) const;

  /**
   * Load forecast data into a FcstGrid
   *
   * @note same as the other methods, but look for a field name that contains the
   *       input wildcard string
   *
   * @param[in] gt  Generation time
   * @param[in] lt  Lead seconds
   * @param[in] proj   Projection information that may be used 
   * @param[in] wildcardFieldName  name that should be a substring of a field
   * @param[out] outGrid  FcstGrid filled with data on exit
   *
   * @return true for success
   *
   * @note queries server
   */
  bool loadFcstDataWithWildcard(const time_t gt, int lt, const ParmProjection &proj,
