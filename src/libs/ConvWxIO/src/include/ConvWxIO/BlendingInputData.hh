...
class BlendingInputData
{
 public:
   ...
   *                        the method
   * @param[in] allowLeadZero  True to allow reading of lead time 0 even if not
   *                           configured to do so
   * @return  true if the FcstGrid was filled, false if not (no more waiting).
   *
   * @note will wait if data is not available until either data becomes 
  ...
  bool fillFcstGridWithWait(const time_t &gt, int lt, int &secondsSpentWaiting,
			    ...
			    bool allowLeadZero=false) const;
  ...
  /**
   * @return true if input generation/lead times have data for the configured
   * parameters, ignores configured for lead times
   * @param[in] gt
   * @param[in] lt
   */
  inline bool hasDataIgnoreConfig(const time_t &gt, int lt) const
  {
    return pParm.hasDataIgnoreConfig(gt, lt);
  }

  inline std::vector <int> getLeadSeconds(const time_t &gt) const
  {
    return pParm.getLtSeconds(gt);
  }
