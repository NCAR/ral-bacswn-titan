...
public:
  ...
  /**
   * Load in netCDF output parameters  by reading a parameter file
   *
   * @param[in] parmFile  Name of the parameter file with netCDF output
   *                      information in it
   * @param[out] netCdfParms  Object to be filled in
   *
   * @return true if all members were successfully parsed
   */
  static bool loadNetCdfParams(const std::string &parmFile,
			       ParmNetCdfOut &netCdfParms);
