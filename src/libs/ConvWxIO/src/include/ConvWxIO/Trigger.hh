...
public:
  ...
  /**
   * Triggering method for ConvWx apps to produce return a generation time
   * of a set of forecasts once all of the forecast lead times are available.
   * A wait will occur before trigger until mandatory latency is met.
   * This waits for new data if there is none.  In this method, the wait is
   * calculated based on another URL's most recent data times in comparision
   * to the triggering URL.
   *
   ...
   *                 Trigger::TRIGGER_OBS data)
   *
   * @return  True if triggering returns a new generation time or false for no more
   */

  static bool genTriggerWLatency(const ParmMain &parms, int latency,
                                 const std::string &dataTimeUrl,
                                 time_t &gt);

  // Real time only, forecasts only, otherwise similar to above
   static bool genTriggerWLatencyUnevenLeads(const ParmMain &parms,
