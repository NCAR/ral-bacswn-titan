...
public:
   ...
   *
   * @param[in] url  Where the data is located
   * @param[in] leads The expected lead times
   * @param[in] debug  True for debugging output
   * @param[out] triggerGenTime    The returned generation time value
   *
   * @return True if successful and triggerGenTime has a meaningful value.
 ...
 /**
  ...
  /**
   * read all the field headers at a URL at a gen/lead time.
   * If some of the fields have long name that is empty, ignore those, and return the short names for
   * all the fields that do not have empty long names
   *
   * @param[in] url
   * @param[in] genTime
   * @param[in] leadTime
   * @return vector of short field names
   */
  static std::vector<std::string> shortNamesIgnoringEmptyLongNames(const std::string &url, const time_t &genTime, int leadTime);
  ...
  static std::vector<int> getAvailableLeads(const time_t &gt, 
                                            const std::string &url,
                                            const int leadSeconds0,
                                            const int leadSeconds1,
                                            const int leadDeltaSeconds);
   ...
   *                  if remap is false, this parameter is not used.
   * @param[in] url  Location of data.
   * @param[in] fieldNum  Number of the particular data at the URL that is wanted
   * @param[in] field  Name to give the field after reading it in
   * @param[in] remap  True if proj should be used to remap the data
   *                   false if data is o.k. as it is stored.
   * @param[out] outGrid  The returned FcstGrid
   *
   ...
   * @note queries a server specified by the URL
   */
  static bool loadFcstByNum(const time_t &gt, const int lt, 
                            const ParmProjection &proj, const std::string &url,
   ...
   *                  if remap is false, this parameter is not used.
   * @param[in] url  Location of data.
   * @param[in] wildcardFieldName  string that should be part of field name to read
   * @param[in] remap  True if proj should be used to remap the data
   *                   false if data is o.k. as it is stored.
   * @param[out] outGrid  The returned FcstGrid
   *
   ...
   * @note queries a server specified by the URL
   */
  static bool loadFcstWithWildcard(const time_t &gt, const int lt, 
                                   const ParmProjection &proj, const std::string &url,
  ...
  /**
   * Writes multiple forecast Grid objects to one output location as MDV,
   * optionally changing long names.
   *
   ...
   * @param[in] dataGrids  The data to write
   * @param[in] metadata  Metadata associated with the data to also write
   * @param[in] shortLongMap   mapping from short names to long names
   *
   * @note All output Grid objects share the same generation and lead time, projection,
   *       and metadata.
   * @note if a short name (in dataGrids) is not in the mapping, the long
   * name is the same as the short name
   */
  ...
  /**
   * Writes multiple forecast Grid objects to one output location as netCDF
   *
   * @param[in] gt  Forecast generation time to give the outputs
   * @param[in] lt  Forecast lead time seconds to give the outputs
   * @param[in] url  Where to write the data
   ...
   * @param[in] dataGrids  The data to write
   * @param[in] metadata  Metadata associated with the data to also write
   * @param[in] nparms  NetCDF output params
   *
   * @note All output Grid objects share the same generation and lead time, projection,
   *       and metadata.
   */
  static void writeNetCDF(const time_t &gt, const int lt, const std::string &url,
			  const ParmProjection &proj, const MultiGrid &dataGrids,
			  const MetaData &metadata, const ParmNetCdfOut &nParms);
  ...
  /**
   * Writes multiple forecast Grid objects to one output location, with
   * thread locking, and optionally changing long names.
   *
   * @param[in] gt  Forecast generation time to give the outputs
   * @param[in] lt  Forecast lead time seconds to give the outputs
   * @param[in] url  Where to write the data
  ...
  /**
   * Set the missing data value used when reading data to the input value
   * Whatever the missing data value actually is gets changed to this value
   * for internal use.  The default missing data value if this method is not
   * called is -99.0
   *
   * @param[in] value  The missing data value to use
   */
  static void setMissingDataValue(double value);

  /**
   * Set a flag about trying to use the missing data value found in input reads as is.
   * If true, it will try to keep those values, if false, if will overwrite with the
   * missing data value that can be set via setMissingDataValue(), which is -99.0 if not set
   *
   * The default if this is not called is true
   *
   * @param[in] value  The status to use
   */
  static void setKeepInputMissingStatus(bool value);
